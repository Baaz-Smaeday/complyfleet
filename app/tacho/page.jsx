    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "5px" }}>
        <span style={{ fontSize: "11px", fontWeight: 700, color: cfg.text }}>
          {status === "never" ? "No download recorded" : status === "overdue" ? `${Math.abs(days)} days overdue` : `${days} days remaining`}
        </span>
...
function LogDownloadModal({ item, type, rule, onClose, onSave }) {
  ...
  async function handleSave() {
    ...
    await onSave({ item, date, software, notes }); setSaving(false); onClose();
  }
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "20px" }} onClick={onClose}>
      <div style={{ background: "#FFF", borderRadius: "20px", width: "100%", maxWidth: "440px", boxShadow: "0 24px 64px rgba(0,0,0,0.2)", overflow: "hidden" }} onClick={e => e.stopPropagation()}>
        <div style={{ padding: "22px 26px", borderBottom: "1px solid #F3F4F6" }}>
          ...
          <div style={{ fontSize: "12px", color: "#64748B", marginTop: "4px" }}>{item.name || item.reg} {item.company_name ? `— ${item.company_name}` : ""}</div>
        </div>
        <div style={{ padding: "22px 26px", display: "flex", flexDirection: "column", gap: "14px" }}>
          ...
          <div>
            ...
            <input type="text" value={software} onChange={e => setSoftware(e.target.value)} placeholder="e.g. Tachomaster, Optac..." style={{ width: "100%", padding: "10px 14px", border: "1px solid #E5E7EB", borderRadius: "10px", fontSize: "14px", outline: "none", fontFamily: "inherit" }} />
          </div>
          <div>
            <label style={{ display: "block", fontSize: "12px", fontWeight: 700, color: "#374151", marginBottom: "6px" }}>Notes (optional)</label>
            <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2} style={{ width: "100%", padding: "10px 14px", border: "1px solid #E5E7EB", borderRadius: "10px", fontSize: "14px", outline: "none", fontFamily: "inherit", resize: "vertical" }} />
          </div>
...
function AddDriverModal({ companies, onClose, onSave }) {
  ...
  async function handleSave() {
    ...
    setSaving(true); await onSave({ name: name.trim(), company_id: companyId, licence_number: licenceNumber.trim() }); setSaving(false); onClose();
  }
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "20px" }} onClick={onClose}>
      <div style={{ background: "#FFF", borderRadius: "20px", width: "100%", maxWidth: "440px", boxShadow: "0 24px 64px rgba(0,0,0,0.2)", overflow: "hidden" }} onClick={e => e.stopPropagation()}>
...
export default function TachoPage() {
  ...
  async function loadData(userProfile) {
    ...
    try {
      ...
      if (userProfile?.role === "tm") {
        // TM: only their linked companies
      ...
      // platform_owner: companyIds stays null = sees everything
      ...
      if (drvs) {
        setDrivers(drvs.map(d => ({
          ...d, company_name: cMap[d.company_id] || "Unknown",
      ...
      if (vehs) {
        setVehicles(vehs.map(v => ({
          ...v, company_name: cMap[v.company_id] || "Unknown",
  ...
  function handlePrintReport() {
    ...
    win.document.write(`<!DOCTYPE html><html><head><title>${title}</title><style>
      ...
      tr.overdue td, tr.never td { background: #FEF2F2; } tr.due-soon td { background: #FFFBEB; }
      .badge { padding: 3px 10px; border-radius: 20px; font-size: 10px; font-weight: 700; display: inline-block; }
      .badge-ok { background: #D1FAE5; color: #065F46; } .badge-overdue, .badge-never { background: #FEE2E2; color: #991B1B; } .badge-due-soon { background: #FEF3C7; color: #92400E; }
      .footer { margin-top: 30px; padding-top: 16px; border-top: 2px solid #E5E7EB; font-size: 10px; color: #6B7280; text-align: center; }
    </style></head><body>
    <div class="header"><div><div class="logo">🛡️ Comply<span>Fleet</span></div><div style="font-size:12px;color:#6B7280;margin-top:4px">${title} — DVSA Compliance Report</div></div><div style="font-size:12px;color:#6B7280">Generated: ${new Date().toLocaleDateString("en-GB")}</div></div>
    <table><thead><tr><th>${activeTab === "drivers" ? "Driver" : "Vehicle"}</th><th>Company</th><th>Last Download</th><th>Next Due</th><th>Status</th><th>Days</th></tr></thead><tbody>
    ${items.map(item => {
      ...
      return `<tr class="${status}"><td style="font-weight:700">${displayName}</td><td>${item.company_name}</td><td>${formatDate(item.last_download)}</td><td>${nextDue}</td><td><span class="badge badge-${status}">${STATUS_CONFIG[status].label}</span></td><td style="font-weight:700">${days === null ? "—" : days < 0 ? `${Math.abs(days)}d overdue` : `${days}d`}</td></tr>`;
    }).join("")}
    </tbody></table>
    <div class="footer">DVSA Compliant Record · ${rule}-day rule · Generated by ComplyFleet · www.complyfleet.co.uk · Cannot be altered</div>
    </body></html>`);
  ...
  function getFiltered(list) {
    ...
    return list.filter(item => {
      if (!isCompanyAdmin && selectedCompany !== "all" && item.company_id !== selectedCompany) return false;
      if (statusFilter !== "all") {
    ...
    <div style={{ minHeight: "100vh", background: "#F1F5F9", fontFamily: "'DM Sans', 'Segoe UI', system-ui, sans-serif" }}>
      ...
      <main style={{ maxWidth: "1200px", margin: "0 auto", padding: "28px 20px" }}>
        ...
        <div style={{ display: "flex", gap: "4px", marginBottom: "20px", background: "#E2E8F0", borderRadius: "14px", padding: "4px", width: "fit-content" }}>
          {[{ key: "drivers", icon: "👤", label: "Driver Cards", sub: "28 day rule" }, { key: "vehicles", icon: "🚛", label: "Vehicle Units", sub: "90 day rule" }].map(tab => (
            <button key={tab.key} onClick={() => { setActiveTab(tab.key); setStatusFilter("all"); }} style={{ padding: "10px 20px", borderRadius: "10px", border: "none", cursor: "pointer", background: activeTab === tab.key ? "#FFF" : "transparent", boxShadow: activeTab === tab.key ? "0 1px 4px rgba(0,0,0,0.1)" : "none", transition: "all 0.2s" }}>
        ...
        {overdueCount > 0 && (
          <div style={{ padding: "14px 20px", borderRadius: "14px", background: "#FEF2F2", border: "2px solid #FECACA", marginBottom: "20px", display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            <div>
              ...
              <div style={{ fontSize: "12px", color: "#DC2626", marginTop: "2px" }}>{overdueCount} {activeTab === "drivers" ? "driver" : "vehicle"}{overdueCount > 1 ? "s" : ""} with overdue or unrecorded downloads</div>
            </div>
          </div>
        )}

        {/* Filters — hide company dropdown for company admin */}
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "20px" }}>
          ...
          {[{ key: "all", label: "All" }, { key: "overdue", label: "🚨 Overdue" }, { key: "due-soon", label: "⏰ Due Soon" }, { key: "ok", label: "✅ OK" }].map(f => (
        ...
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: "80px", color: "#94A3B8" }}>
            ...
            <div style={{ fontWeight: 800, fontSize: "16px", color: "#374151", marginBottom: "8px" }}>{activeTab === "drivers" ? "No drivers found" : "No vehicles found"}</div>
            <div style={{ fontSize: "13px", marginBottom: "24px" }}>{activeTab === "drivers" ? "Add drivers to start tracking their tacho downloads" : "Add vehicles first from the Companies page"}</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: "16px" }}>
            {filtered.map((item, i) => (
              <ComplianceCard key={item.id} name={activeTab === "drivers" ? item.name : item.reg} subtitle={activeTab === "drivers" ? item.company_name : `${item.company_name} · ${item.make || ""} ${item.model || ""}`.trim()} lastDownload={item.last_download} rule={rule} statusIcon={activeTab === "drivers" ? undefined : "🚛"} onLog={() => setLogItem(item)} onHistory={() => setHistoryItem(item)} animDelay={i * 60} />
            ))}
          </div>
        )}
      </main>
