...
export default function LoginPage() {
  ...
  async function handleSignup(e) {
    ...
    if (err) { setError(err.message); setLoading(false); return; }
    if (data.user && !data.user.identities?.length) { setError("An account with this email already exists"); setLoading(false); return; }
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0F172A 0%, #1E293B 50%, #0F172A 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <div style={{ width: "100%", maxWidth: "440px" }}>
        <div style={{ textAlign: "center", marginBottom: "32px" }}>
          <div style={{ width: "64px", height: "64px", borderRadius: "18px", background: "linear-gradient(135deg, #3B82F6, #2563EB)", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: "32px", marginBottom: "16px", boxShadow: "0 8px 32px rgba(37,99,235,0.3)" }}>🚛</div>
        ...
        <div style={{ background: "white", borderRadius: "20px", padding: "32px", boxShadow: "0 24px 64px rgba(0,0,0,0.3)" }}>
          ...
          {mode === "signup" && (
            <form onSubmit={handleSignup}>
              ...
              <button type="submit" disabled={loading} style={{ ...btnStyle, opacity: loading ? 0.6 : 1 }}>{loading ? "Creating account..." : "Create Account"}</button>
            </form>
          )}
          {mode === "forgot" && (
            <form onSubmit={handleForgotPassword}>
              ...
              <button type="submit" disabled={loading} style={{ ...btnStyle, opacity: loading ? 0.6 : 1 }}>{loading ? "Sending..." : "Send Reset Link"}</button>
            </form>
          )}
          <div style={{ textAlign: "center", marginTop: "20px", paddingTop: "20px", borderTop: "1px solid #F3F4F6" }}>
            {mode === "login" && (
              <span style={{ fontSize: "13px", color: "#64748B" }}>Don't have an account?{" "}
            ...
            {(mode === "signup" || mode === "forgot") && (
              <span style={{ fontSize: "13px", color: "#64748B" }}>Already have an account?{" "}
        ...
        <div style={{ textAlign: "center", marginTop: "24px", color: "#64748B", fontSize: "11px" }}>ComplyFleet · DVSA Compliance Platform · © 2026</div>
      </div>
    </div>
  );
}
