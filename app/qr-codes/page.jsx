...
function QRCard({ vehicle, baseUrl }) {
  ...
  const handlePrint = () => {
    ...
    win.document.write(`<!DOCTYPE html><html><head><title>QR - ${vehicle.reg}</title><style>
      ...
      body { font-family: 'DM Sans', sans-serif; display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; background: white; }
      .card { text-align: center; padding: 40px 36px; border: 3px solid #0F172A; border-radius: 20px; width: 320px; }
      .logo { font-size: 15px; font-weight: 800; color: #0F172A; margin-bottom: 20px; letter-spacing: -0.02em; }
      .logo span { color: #2563EB; }
      .reg { font-size: 34px; font-weight: 800; font-family: monospace; color: #0F172A; margin: 16px 0 4px; letter-spacing: 2px; }
      .type { font-size: 14px; color: #6B7280; margin-bottom: 2px; }
      .company { font-size: 12px; color: #94A3B8; }
      .scan { margin-top: 18px; padding: 12px 20px; background: #0F172A; color: white; border-radius: 10px; font-size: 13px; font-weight: 800; letter-spacing: 0.03em; display: inline-block; }
      .footer { font-size: 10px; color: #CBD5E1; margin-top: 14px; font-weight: 500; }
      @media print { body { -webkit-print-color-adjust: exact; print-color-adjust: exact; } }
    </style></head><body><div class="card">
      <div class="logo">🚛 Comply<span>Fleet</span></div>
      ...
      <div class="scan">📱 SCAN FOR DAILY WALKAROUND CHECK</div>
      <div class="footer">DVSA Compliant · Scan with any phone camera · No app needed</div>
    </div></body></html>`);
    win.document.close();
...
export default function ComplyFleetQRCodes() {
  ...
  const handlePrintAll = () => {
    ...
    let html = `<!DOCTYPE html><html><head><title>ComplyFleet QR Codes</title><style>
      @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;700;800&display=swap');
      body { font-family: 'DM Sans', sans-serif; margin: 0; padding: 20px; background: white; }
      .grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; }
      .card { text-align: center; padding: 24px; border: 2px solid #0F172A; border-radius: 16px; page-break-inside: avoid; }
      .reg { font-size: 24px; font-weight: 800; font-family: monospace; margin: 10px 0 4px; letter-spacing: 1px; }
      .type { font-size: 12px; color: #6B7280; }
      .scan { margin-top: 10px; padding: 8px 14px; background: #0F172A; color: white; border-radius: 8px; font-size: 11px; font-weight: 700; display: inline-block; }
      .footer { font-size: 9px; color: #CBD5E1; margin-top: 8px; }
      @media print { body { -webkit-print-color-adjust: exact; print-color-adjust: exact; } }
    ...
    filtered.forEach(v => {
      ...
      html += `<div class="card"><img src="${qr}" width="130" height="130" /><div class="reg">${v.reg}</div><div class="type">${v.type} · ${v.make || ""} ${v.model || ""}</div><div class="scan">📱 SCAN FOR WALKAROUND CHECK</div><div class="footer">DVSA Compliant · Scan with any phone camera</div></div>`;
    });
    html += `</div></body></html>`;
      ...
      <main style={{ maxWidth: "1400px", margin: "0 auto", padding: "24px 20px" }}>
        ...
        <div style={{ padding: "16px 20px", borderRadius: "14px", background: "#EFF6FF", border: "1px solid #BFDBFE", marginBottom: "20px", display: "flex", gap: "12px", alignItems: "center" }}>
          ...
          <div style={{ fontSize: "13px", color: "#1E40AF", lineHeight: 1.5 }}>
            <strong>How it works:</strong> Each QR code links directly to the walkaround check for that specific vehicle. Print, laminate, and stick one in each cab. Drivers scan with their phone camera — no app needed.
          </div>
        </div>
